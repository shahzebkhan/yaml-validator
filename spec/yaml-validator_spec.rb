  ...
  describe "#validate" do
    ...
    describe "wrong_variables scenario" do
      it "returns two errors" do
        validator = YamlValidator.new('spec/fixtures/wrong_variables', 'foo.en.yml', 'bar.es.yml')
        errors = validator.validate()
        errors.should == [
          "bar.es.yml: translations.parent1.key1: missing variable 'name1' (available options: name, day_of_week)", 
          "bar.es.yml: translations.parent1.key1: missing variable 'day_of_week1' (available options: name, day_of_week)"
        ]
      end
    end
    ...
    describe "missing translations" do
      it "returns invalid yaml error" do
        validator = YamlValidator.new('spec/fixtures/missing_translations', 'foo.en.yml', 'bar.es.yml', :show_missing => true)
        errors = validator.validate()
        errors.should == [
          "bar.es.yml: missing translation for translations.key2 ('value2')", 
          "bar.es.yml: missing translation for translations.parent2.key3 ('value3')", 
          "bar.es.yml: missing translation for translations.parent3.2 ('three')", 
          "bar.es.yml: missing translation for translations.parent3.3 ('')"
        ]
      end
    end
